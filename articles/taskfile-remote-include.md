...
2025年にこの記事を書いていたのですが、**2026年08月にリリースしたv3.53.1からGAとなりました！**

https://x.com/taskfiledev/status/2089770399156338959

https://taskfile.dev/blog/remote-taskfiles

普段使いしている限りではGA以前のファイルがそのまま問題なく使用可能そうでした。
もしかしたら挙動まわりに若干の差異は出ている可能性はあるので、その点には注意してください。

:::details 以前から使っていた人向け
更新前の記事には `事前作業: 実験的要素の有効化を行う` というセクションがあり、
環境変数等で「実験的機能を有効にする」ための設定方法について説明していました。

今回のリリースから、この設定を検知すると逆に下記のメッセージが出力されるようになっています。

```text:taskコマンド実行時の出力
task: Experiment "REMOTE_TASKFILES" is released and no longer needs to be enabled
```

見かけたら、残っている設定記述を消しておきましょう。
:::
...
このインクルード機能ですが、リモートのファイルもインクルードできます。
