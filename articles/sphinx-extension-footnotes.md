...
`1.` でロードした処理などは、後続プロセスにおいてSphinxのイベント呼び出しによって実行することができます。
...
例えばZennのMarkdown記法では、脚注記法である `[^X]` + `[^X]:` の組み合わせは、
どこに書かれていても脚注として扱われ、脚注本文は文末に表示されます。 [^3]
...
[^3]: 実際にこの記事でも、脚注はセクション末尾に書くようにしています。
...
Sphinxで使用する`conf.py`は、設定項目だけではなく関数などを定義して処理の追加定義をすることが可能です。
...
ビルドの途中でビルド対象となるreSTファイルのフルパスが羅列されると思います。[^5]
これは、上記のコードが働いた結果として、次のことがビルド時に発生しているためです。

1. `conf.py`に`setup()`があるので、ビルド初期に実行。
1. `setup()`内にコアイベント時の処理を登録する`app.connect()`が記述されており、`doctree-read`=ソースを読み込んでオブジェクトにした後に追加処理する関数を指定。
...
ここから、実際に脚注を表す箇所を文末に集約させるコードを書いてみます。
...
1. 見つかった要素は、元の親要素への参照を除外た後に、空セクションの子要素に追加させて、場所移動させる。
1. 最後に空だった「全`footnote`が集まったセクション」を`doctreeeの子要素最後尾に挿入して、文末に出力されるようにする。
...
とはいえ、Sphinxドキュメントを書くたびに毎回`conf.py`にこの記述を書くのも少々煩雑です。[^6]
...
これだけで、今回のテーマである「脚注を参照元の近くに書いた上で、ビルド時に文末へ寄せる」ととが実現できます。
